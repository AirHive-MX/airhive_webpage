      ...
      <section className="ah-container grid gap-6 py-8 md:grid-cols-2">
        {[{ key: "mission", text: t("about.mission_text") }, { key: "vision", text: t("about.vision_text") }].map((item) => (
          <motion.article
            ...
            className="ah-blue-card rounded-2xl border-2 p-7 transition duration-500 sm:p-8 bg-white shadow-[0_14px_34px_rgba(42,71,246,0.18)]"
            style={{ borderColor: 'rgba(42, 71, 246, 0.55)' }}
          >
            <p className="text-base font-bold uppercase tracking-[0.24em] sm:text-lg text-[#2A47F6]">{t(`about.${item.key}_button`)}</p>
            <p className="mt-4 text-lg leading-relaxed sm:text-xl text-[#202020]/74">{item.text}</p>
          </motion.article>
        ))}
      </section>
